...
public interface KhachHangLeadRepo extends JpaRepository<KhachHangLead, String> {
    ...
    String query = "select khl from KhachHangLead khl where coalesce(khl.ten, '') like %?1% and coalesce(khl.phone, '') like %?2% and coalesce(khl.phone2, '') like %?3% and coalesce(khl.soThe, '') like %?4% and coalesce(khl.email, '') like %?5% and coalesce(khl.diaChi, '') like %?6% and coalesce(khl.facebook, '') like %?7% and coalesce(khl.zalo, '') like %?8% and coalesce(khl.gioiTinh, '') like %?9% and coalesce(khl.thanhPho, '') like %?10% and coalesce(khl.quanHuyen, '') like %?11% and coalesce(khl.phuongXa, '') like %?12%";
    @Query(query)
